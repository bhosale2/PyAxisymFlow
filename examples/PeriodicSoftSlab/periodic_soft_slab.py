...
from pyaxisymflow.kernels.compute_velocity_from_psi import compute_velocity_from_psi_periodic
from pyaxisymflow.kernels.compute_vorticity_from_velocity import compute_vorticity_from_velocity_periodic
...
from pyaxisymflow.kernels.advect_vorticity_via_eno3 import gen_advect_vorticity_via_eno3_periodic
from pyaxisymflow.kernels.FastDiagonalisationStokesSolver import FastDiagonalisationStokesSolver
from pyaxisymflow.kernels.diffusion_RK2 import diffusion_RK2_periodic
from pyaxisymflow.kernels.periodic_boundary_ghost_comm import gen_periodic_boundary_ghost_comm, gen_periodic_boundary_ghost_comm_eta
from pyaxisymflow.elasto_kernels.div_tau import update_vorticity_from_solid_stress_periodic
from pyaxisymflow.elasto_kernels.solid_sigma import solid_sigma_periodic
from pyaxisymflow.elasto_kernels.advect_refmap_via_eno3 import gen_advect_refmap_via_eno3_periodic
...
vort_double = 0 * Z_double
vort_particles = 0 * vort_double
Z1_double, R1_double = np.meshgrid(z-2*ghost_size, z-2*ghost_size)
eta1 = Z.copy()
eta2 = R.copy()
eta1_double = Z1_double.copy()
eta2_double = R1_double.copy()
u_z_double = R1_double.copy()
...
Y,vel_sl,vel_fl=theory_axisymmetric_soft_slab_spatial(L_f,L_s,Re,shear_rate,omega,nu,G,V_wall)
...
while t < tEnd:
    ...
    #print(np.amax((eta1[:, int(grid_size_z / 2)]-eta1_old[:, int(grid_size_z / 2)])/dt))
    # plotting!!
    if freqTimer >= freqTimer_limit:
        freqTimer = 0.0

        plt.plot(R[:int(grid_size_r / 2), int(grid_size_z / 2)], u_z[:int(grid_size_r / 2), int(grid_size_z / 2)])
        # plt.plot(R[:, int(grid_size_z / 2)],(eta1[:, int(grid_size_z / 2)]-eta1_old[:, int(grid_size_z / 2)])/dt)
        # # plt.plot(
        # #     y_range,
    ...
    extrap_LS(dx, grid_size_r, grid_size_z, eps, eta1,-ball_phi, extrap_tol, extrap_zone)
    extrap_LS(dx, grid_size_r, grid_size_z, eps, eta2,-ball_phi, extrap_tol, extrap_zone)
    ...
    update_vorticity_from_solid_stress_periodic(
        vorticity, tau_z, tau_r, sigma_s_11, sigma_s_12, sigma_s_22, R, dt, dx, per_communicator1
    ) 
    ...
    # diffuse vorticity
    diffusion_RK2_periodic(vorticity, temp_vorticity, R, nu, dt, dx, per_communicator1)
    #  update time
    t += dt
    freqTimer += dt
    it += 1
