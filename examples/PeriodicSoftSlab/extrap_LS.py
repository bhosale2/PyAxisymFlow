...
def gen_extrap_LS_const(
    dx, grid_size_r, grid_size_z, eps, num_threads=False
):
    ...
    shift_p = spp.diags(
        [1, 1], [1, -grid_size_r+ 1], shape=(grid_size_r, grid_size_r), format="csr",
    )

    shift_p_T = spp.diags(
        [1, 1], [1, -grid_size_z+ 1], shape=(grid_size_z, grid_size_z), format="csr",
    )

    shift_pp = spp.diags(
        [1, 1], [2, -grid_size_r + 2], shape=(grid_size_r, grid_size_r), format="csr",
    )

    shift_pp_T = spp.diags(
        [1, 1], [2, -grid_size_z + 2], shape=(grid_size_z, grid_size_z), format="csr",
    )
    ...
    def extrap_LS_const(eta, phi, extrap_tol, extrap_band):
        """
        ...
        """
        extrapolate field: H(psi + h) phi_n * eta = 0
        ...
        return extrap_LS_const




def extrap_LS(dx, grid_size_r, grid_size_z, eps, eta, phi, extrap_tol, extrap_band, num_threads=False):
    ...
    while res > extrap_tol:
        ...
        extrap_num_y = dir_yp * (
            2 * shift_n * eta - 0.5 * shift_nn * eta
        ) - dir_yn * (2 * shift_p * eta - 0.5 * shift_pp * eta)
        extrap_num_x = dir_xp * (
