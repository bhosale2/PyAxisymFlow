...
density = (rho_s - rho_f) * part_char_func + rho_f
...
while t < tEnd:
    ...
    M_proj = np.sum(density * part_char_func * u_z * R)
    ...
    smooth_Heaviside(part_char_func, part_phi, moll_zone)
    density[...] = (rho_s - rho_f) * part_char_func + rho_f
    part_mass = np.sum(density * part_char_func * R)
    # U_z_cm_part = np.sum(density * part_char_func * u_z * R) / part_mass
    ...
    update_baroclinic_vorticity(vorticity, u_z, u_r, old_u_z, old_u_r, density, dt, dx)
    ...
    diffusion_RK2_unb_diffrho(vorticity, 0 * temp_vorticity, R, density, nu, dt, dx)
